...
```{r}
rawDataSept <- read.csv(file = "sampleSept2021.csv")
...
```{r}
#create a corpus:
importdocs = corpus(rawDataSept, text_field = 'text')
```
...
```{r}
saveRDS(mycorpus, file = "mycorpusSept2021.rds")

#mycorpus <- read_rds("mycorpusSept2021.rds")
```
...
end_time - start_time # time lapsed
#print(mysentiment_memory, expr = FALSE)

#Giga Bytes used:
sum((mysentiment_memory$bytes), na.rm=TRUE)/1000000000 # Giga bytes used
...
sentimentResultSept2021 <- rawDataSept

#add sentiment score:
sentimentResultSept2021$sentiment_Syuzhet = mysentiment
