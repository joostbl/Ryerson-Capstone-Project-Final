...
data_set_sept  <- data.table::fread("corona_tweets_544 Sept 2021", header = T, encoding = "Latin-1")

glimpse(data_set_sept )

head(data_set_sept,10)
...
set.seed(1000)
sample <- data_set_sept[sample(nrow(data_set_sept), size = 5000), ]
...
news_term <- covid_clean %>% 
   ...
   mutate(word = ifelse(word == "realdonaldtrump", "trump", word),
          word = ifelse(word == "vaccinate", "vaccine", word),
...
#lda_model$get_top_words(n = 3, topic_number = c(1L, 3L), lambda = 0.2)
